...
class BookDao {
  ...
  Future<List<Book>> getAll() async {
    final db = await _db.database;
    return (await db.query("books", orderBy: '"order" ASC, latestChapterTime DESC')).map((m) => Book.fromMap(m)).toList();
  }
  Future<List<Book>> getByGroup(int group) async {
    final db = await _db.database;
    return (await db.query("books", where: '"group" = ?', whereArgs: [group], orderBy: '"order" ASC')).map((m) => Book.fromMap(m)).toList();
  }
  Future<bool> has(String bookUrl) async {
    final db = await _db.database;
    return (Sqflite.firstIntValue(await db.rawQuery("SELECT COUNT(*) FROM books WHERE bookUrl = ?", [bookUrl])) ?? 0) > 0;
  }
  Future<int> count() async {
    final db = await _db.database;
    return Sqflite.firstIntValue(await db.rawQuery("SELECT COUNT(*) FROM books")) ?? 0;
  }
  Future<void> deleteAll() async { final db = await _db.database; await db.delete("books"); }
}
